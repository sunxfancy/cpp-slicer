...
Statement::defsMap Statement::setDataEdgesRec(const defsMap& parent_def_map, std::vector<Statement*> loopRefs,
                                              int inABranch) {
    defsMap def_map;
    // make every parent definition edge true.

    for (auto& ds : parent_def_map) {
        for (auto& d : ds.second) def_map[ds.first].insert({d.first, Edge::None});
    }
    if (name() == Type::Loop) {
    ...
    for (auto loopIteration = 0; loopIteration < 2; ++loopIteration) {
            ...
            for (auto& def : stmt.first->getDefine()) {
                ...
                if (def_map.find(def) != def_map.end()) {
                    for (auto defStmt = def_map[def].begin(); defStmt != def_map[def].end();) {
                        ...
                        if (inABranch > 0) {
                            if (defStmt->second == stmt.second && defStmt->second != Edge::None) {
            ...
            if (!stmt.first->controlChildren.empty()) {
                ...
                for (auto& defs : def_map) {
                    for (auto def : defs.second) {
                        if (def.second == Edge::None || def.second == stmt.second || loopIteration > 0) {
                ...
                if (inABranch > 0) {
                    for (auto& kv : child_new_defs) {
                        for (auto& v : kv.second) {
                            if (v.second != Edge::None) def_map[kv.first].insert({v.first, stmt.second});
                        }
                    }
                } else {
                    for (auto& kv : child_new_defs) {
                        if (kv.second.size() > 1) {
                            ...
                            //  this is temporarily fixed as 2, but for implementing switch-cases, we need a smarter
                            //  solution.
                            if (kv.second.size() > 2) {
        ...
        for (auto it = def_map.begin(); it != def_map.end();) {
            for (auto it2 = it->second.begin(); it2 != it->second.end();) {
                if (llvm::isa<clang::DeclStmt>(it2->first->getAstRef()) && it2->second != Edge::None) {
...
std::string Statement::EdgeToStr(Edge e) {
    switch (e) {
        ...
        case Edge::False: return "F";
        case Edge::True: return "T";
        default: return "";
    }
}
...
std::string Statement::stmt2str(const clang::Stmt* s, clang::SourceManager& sm) {
    // (T, U) => "T,,"
    auto text = clang::Lexer::getSourceText(clang::CharSourceRange::getTokenRange(s->getSourceRange()), sm,
                                            clang::LangOptions(), nullptr)
                    .str();
    if (text.at(text.size() - 1) == ',') {
        return clang::Lexer::getSourceText(clang::CharSourceRange::getCharRange(s->getSourceRange()), sm,
                                           clang::LangOptions(), nullptr)
            .str();
    }
    return text;
...
std::string Statement::sourceString(clang::SourceManager& sm) { return stmt2str(astRef, sm); }
...
std::string CompoundStatement::sourceString(clang::SourceManager& /*sm*/) { return "{}"; }
...
std::string Statement::dumpDotRec(clang::SourceManager& sm, bool markSliced, std::map<int, std::vector<int>>& rank_map,
                                  int depth) {
    ...
    for (auto& c : controlChildren) {
        ret += std::to_string(id) + " -> " + std::to_string(c.first->getId()) + "[label=\"" + EdgeToStr(c.second) +
               "\",style=bold";
