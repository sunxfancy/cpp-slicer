...
AST_MATCHER(UnaryOperator, isIncrementDecrementOp) { return Node.isIncrementDecrementOp(); }
...
void PDGBuilder::setSlicingStmt(const MatchFinder::MatchResult& result, const Stmt* astRef) {
    ...
    if (start.getSpellingLineNumber() <= lineNo && start.getSpellingColumnNumber() <= colNo &&
        lineNo <= end.getSpellingLineNumber() && colNo <= end.getSpellingColumnNumber() &&
        slicePos.refined(start.getSpellingLineNumber(), start.getSpellingColumnNumber(), end.getSpellingLineNumber(),
...
void PDGBuilder::registerMatchers(MatchFinder* MatchFinder) {
    ...
    const auto decls =
        declStmt(forEachDescendant(
                     varDecl(anyOf(hasInitializer(stmt(forEachDescendant(declRefExpr(to(varDecl().bind("declInit")))))),
                                   hasInitializer(anything()), unless(hasInitializer(anything()))))
                         .bind("decl")))
            .bind("declStmt");

    const auto bop = binaryOperator(isAssignmentOp(), hasLHS(ignoringImpCasts(declRefExpr(to(varDecl().bind("lval"))))),
                                    anyOf(hasRHS(forEachDescendant(expr(declRefExpr(to(varDecl().bind("rval")))))),
                                          hasRHS(anything())))
                         .bind("binop");

    // fixme do something with unary ops which are embedded in statements like: int x = a++;
    const auto uop =
        unaryOperator(isIncrementDecrementOp(), hasDescendant(declRefExpr(to(varDecl().bind("uval"))))).bind("unop");

    const auto ret = returnStmt(forEachDescendant(declRefExpr(to(varDecl().bind("retVar"))))).bind("ret");
    // branch
    const auto ifs =
        ifStmt(hasCondition(anyOf(forEachDescendant(declRefExpr(to(varDecl().bind("ifCondVar")))), anything())))
            .bind("if");
    // loop
    // todo detect and handle continue/break statements + the big question: should we?
    const auto whiles =
        whileStmt(hasCondition(anyOf(forEachDescendant(declRefExpr(to(varDecl().bind("whileCondVar")))), anything())))
            .bind("while");
    // compound
    ...
    MatchFinder->addMatcher(
        functionDecl(hasName(funcName), eachOf(forEachDescendant(ifs), forEachDescendant(whiles),
                                               forEachDescendant(compounds), forEachDescendant(decls),
                                               forEachDescendant(bop), forEachDescendant(uop), forEachDescendant(ret)))
            .bind("f"),
        this);
}
