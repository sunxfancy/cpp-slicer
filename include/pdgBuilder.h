...
public:
    ...
    explicit PDGBuilder(std::string _funcName, int _lineNo, int _colNo, bool _dumpDot)
        : funcName(std::move(std::move(_funcName))), lineNo(_lineNo), colNo(_colNo), dumpDot(_dumpDot) {}
...
private:
    bool hasStmt(const Stmt* value) { return stmt_map.find(value) != stmt_map.end(); }
    ...
    std::unordered_multimap<unsigned, Statement*> stmts;
};
}  // namespace clang::slicer
#endif  // PDGBUILDER_H
